...
package object ergomix {
  ...
  case class FullMixBox(inputBox: InputBox) extends MixBox(inputBox) {
    val (r4, r5) = (getR4.getValue, getR5.getValue) match {
      case (c1:GroupElement, c2:GroupElement) => (c1, c2)//Values.GroupElementConstant(c1), Values.GroupElementConstant(c2)) => (c1, c2)
      case (r4, r5) => throw new Exception(s"Invalid registers R4:$r4, R5:$r5")
    }
  }
  ...
  trait FullMixBoxSpender {
    def spendFullMixBox(f: FullMixBox, endBoxes: Seq[EndBox], feeAmount:Long, otherInputBoxIds:Array[String], changeAddress:String, additionalDlogSecrets:Array[BigInteger], additionalDHTuples:Array[DHT])(implicit ctx:BlockchainContext): SignedTransaction =
      spendFullMixBox(f, endBoxes, feeAmount, ctx.getBoxesById(otherInputBoxIds:_*), changeAddress, additionalDlogSecrets, additionalDHTuples)

    def spendFullMixBox(f: FullMixBox, endBoxes: Seq[EndBox], feeAmount:Long, otherInputBoxes:Array[InputBox], changeAddress:String, additionalDlogSecrets:Array[BigInteger], additionalDHTuples:Array[DHT]): SignedTransaction

    def spendFullMixBoxNextAlice(f: FullMixBox, nextX:BigInteger, feeAmount:Long, otherInputBoxes:Array[InputBox], changeAddress:String, additionalDlogSecrets:Array[BigInteger], additionalDHTuples:Array[DHT])(implicit ctx: BlockchainContext): HalfMixTx = {
      ...
      val signedTransaction = spendFullMixBox(f, Seq(endBox), feeAmount, otherInputBoxes, changeAddress, additionalDlogSecrets, additionalDHTuples)
      HalfMixTx(signedTransaction)
    }

    def spendFullMixBoxNextAlice(f: FullMixBox, nextX:BigInteger, feeAmount:Long, otherInputBoxIds:Array[String], changeAddress:String, additionalDlogSecrets:Array[BigInteger], additionalDHTuples:Array[DHT])(implicit ctx: BlockchainContext): HalfMixTx =
      spendFullMixBoxNextAlice(f, nextX, feeAmount, ctx.getBoxesById(otherInputBoxIds:_*), changeAddress, additionalDlogSecrets, additionalDHTuples)

    def spendFullMixBoxNextBob(f: FullMixBox, h:HalfMixBox, nextY:BigInteger, feeAmount:Long, otherInputBoxIds:Array[String], changeAddress:String, additionalDlogSecrets:Array[BigInteger], additionalDHTuples:Array[DHT])(implicit ctx: BlockchainContext): (FullMixTx, Boolean) =
      spendFullMixBoxNextBob(f, h, nextY, feeAmount, ctx.getBoxesById(otherInputBoxIds:_*), changeAddress, additionalDlogSecrets, additionalDHTuples)

    def spendFullMixBoxNextBob(f: FullMixBox, h:HalfMixBox, nextY:BigInteger, feeAmount:Long, otherInputBoxes:Array[InputBox], changeAddress:String, additionalDlogSecrets:Array[BigInteger], additionalDHTuples:Array[DHT])(implicit ctx: BlockchainContext): (FullMixTx, Boolean) = {
      implicit val ergoMix = new ErgoMix(ctx)
      ...
      val outBox1 = EndBox(ergoMix.fullMixScriptErgoTree, Seq(ErgoValue.of(c1), ErgoValue.of(c2)), ErgoMix.mixAmount)
      val outBox2 = EndBox(ergoMix.fullMixScriptErgoTree, Seq(ErgoValue.of(c2), ErgoValue.of(c1)), ErgoMix.mixAmount)
      val endBoxes = Seq(outBox1, outBox2)
      val tx: SignedTransaction = spendFullMixBox(f, endBoxes, feeAmount, inputBoxes, changeAddress, additionalDlogSecrets, DHT(g, gX, gY, gXY, nextY) +: additionalDHTuples)
  ...
  case class FeeEmissionTx(tx:SignedTransaction, gZ:GroupElement, address:ErgoAddress)(implicit ergoMix: ErgoMix) {
    ...
    require(getFeeEmissionBox.inputBox.getErgoTree == ergoMix.feeEmissionContract(gZ).getErgoTree)
  }
